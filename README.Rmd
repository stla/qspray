...
the style of `x^2.y.z^3` if there are at most three variables, otherwise they 
are printed like `x1^2.x2.x3^3`: 
...
With `showQsprayX1X2X3`, you choose only one letter for the variables which 
will be indexed:
...
to use it in your package, include the following instruction in the 
DESCRIPTION file:
...
operators `+`, `-`, `*` and the power for the objects of type `Qspray<T>` 
as long as the operators `+`, `-` and `*` are available for the type `T`. 
So you don't have to implement the arithmetic operations for the 
`Qspray<double>` polynomials if you instantiate this type.
...
`std::vector<int>` objects and whose values are `T` objects. A key represent 
a monomial, e.g. the vector `{2,1,3}` represents the monomial `x^2*y*z^3`, and 
a value represents the coefficient attached to this monomial.
