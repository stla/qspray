...
If you want numerical approximations in the results, use the option `N=TRUE`:
...
is to say a function appropriate for the `"showQspray"` option, must return 
a function which transforms a `qspray` to a string
There is a couple of helper functions to construct a `showQspray` function. 
...
number by an application of the function `gmp::as.bigq`.
...
greatest common divisor of two `qspray` objects)
